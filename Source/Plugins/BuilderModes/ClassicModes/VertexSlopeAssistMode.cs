...
namespace CodeImp.DoomBuilder.BuilderModes
{
	...
	public sealed class VertexSlopeAssistMode : BaseClassicMode
	{
		...
		private const float PREV_THICKNESS = 0.6f;
		private const float ERRL_THICKNESS = 1.8f;
		private const int MODE_SELECT = 0;
		private const int MODE_ASSIGNGROUPS = 1;
		private const int MODE_RELPOINTS = 4; // This mode doesn't exist but tell which modes begin to realloc points
		private const int MODE_NEWGROUP = 4;
		private const int MODE_MODGROUP = 5;
		private const int MODE_NEWPOINTS = 6; // This mode doesn't exist but tell which modes begin to create new points
		private const int MODE_NEWTAGGEDTRIANGLE = 6;
		private const int MODE_NEWEMPTYTRIANGLE = 7;
		...
		{
			vertexnames = new TextLabel[8];
			for (int i = 0; i < 8; i++)
			{
				vertexnames[i] = new TextLabel(1);
				vertexnames[i].TransformCoords = true;
				vertexnames[i].Rectangle = new RectangleF(0f, 0f, 0f, 0f);
				vertexnames[i].AlignX = (i >= 4) ? TextAlignmentX.Left : TextAlignmentX.Center;
				vertexnames[i].AlignY = TextAlignmentY.Middle;
				vertexnames[i].Scale = (i >= 4) ? 16f : 18f;
				vertexnames[i].Color = (i >= 4) ? General.Colors.ModelWireframe.WithAlpha(240) : General.Colors.Vertices.WithAlpha(255);
		...
		public void UpdateModeText()
		{
			...
			{
				BuilderPlug.Me.VertexSlopeAssistForm.ModeText = string.Format("Select on Groups list or click vslope for point {0}", indexedTags.Count + 1);
			}
			...
			if (things != null && things.Count == 1)
			{
				...
				if (indexedTags.Count >= 3)
				{
					if (indexedLinedef != null)
					{
						...
						BuilderPlug.Me.VertexSlopeAssistForm.SetupForm();
		...
		public void CB_TagGroup_DoubleClick(object sender, EventArgs eventArgs)
		{
			List<Thing> things = BuilderPlug.Me.VertexSlopeAssistForm.GetThingsList();
			...
			// Make a view area from all things that makes the selected vslope
			foreach (Thing t in things) area = MapSet.IncreaseArea(area, t.Position);
		...
		public override void OnEngage()
		{
			...
			BuilderPlug.Me.VertexSlopeAssistForm.OnGroupDoubleClick += CB_TagGroup_DoubleClick;
		...
		public override void OnRedrawDisplay()
		{
			...
			DrawnVertex curp = DrawGeometryMode.GetCurrentPosition(mousemappos, snaptonearest, snaptogrid, snaptocardinaldirection, renderer, points);
			if (curp.pos.IsFinite()) curmousepos = curp.pos;

			// Get selected tag things
			List<Thing> things = BuilderPlug.Me.VertexSlopeAssistForm.GetThingsList();
			bool verValid = (things != null && things.Count == 1);
			bool triValid = (things != null && things.Count == 3);
			PixelColor triColor = triValid ? General.Colors.Highlight : General.Colors.InvalidSelect;
			PixelColor valColor = (verValid || triValid || (mode >= MODE_NEWPOINTS)) ? General.Colors.Highlight : General.Colors.InvalidSelect;

			// Get visuals info
			bool showheightz = BuilderPlug.Me.VertexSlopeAssistForm.ShowHeightZ;
			bool showvsactions = BuilderPlug.Me.VertexSlopeAssistForm.ShowVSlopeActions;
			...
			{
				// Show all linedefs actions VSlopes
				if (showvsactions)
				{
					List<Linedef> ldPrevErr = BuilderPlug.Me.VertexSlopeAssistForm.GetVSlopeActionError();
					if (ldPrevErr != null)
					{
						foreach (Linedef ld in ldPrevErr)
						{
							renderer.RenderLine(ld.Start.Position, ld.End.Position, ERRL_THICKNESS, General.Colors.InvalidLinedef, true);
						}
					}
					List<List<Thing>> ldPrevLst = BuilderPlug.Me.VertexSlopeAssistForm.GetVSlopeActionList();
					if (ldPrevLst != null)
					{
						PixelColor colorprev = General.Colors.PreviewLinedef;
						foreach (List<Thing> lt in ldPrevLst)
						{
							renderer.RenderLine(lt[0].Position, lt[1].Position, PREV_THICKNESS, colorprev, true);
							renderer.RenderLine(lt[1].Position, lt[2].Position, PREV_THICKNESS, colorprev, true);
							renderer.RenderLine(lt[2].Position, lt[0].Position, PREV_THICKNESS, colorprev, true);
						}
					}
				}

				// Mouse over tag
				if (mode < MODE_RELPOINTS)
				{
				...
				if (mode == MODE_SELECT && things != null)
				{
					if (things.Count >= 2)
					{
						for (int i = 1; i < things.Count; i++)
							renderer.RenderLine(things[i - 1].Position, things[i].Position, LINE_THICKNESS, triColor, true);
						renderer.RenderLine(things[things.Count - 1].Position, things[0].Position, LINE_THICKNESS, triColor, true);
						if (triValid)
						{
							for (int i = 0; i < 3; i++)
							{
								...
								tl.Rectangle = new RectangleF(things[i].Position.x, things[i].Position.y, 0f, 0f);
								tl.Text = (i + 1).ToString();
								renderer.RenderRectangleFilled(new RectangleF(t.Position.x - vsize, t.Position.y - vsize, vsize * 2.0f, vsize * 2.0f), triColor, true);
								renderer.RenderText(tl);
								if (showheightz)
								{
									TextLabel tl2 = vertexnames[i + 4];
									tl2.Rectangle = new RectangleF(things[i].Position.x, things[i].Position.y, 0f, 0f);
									tl2.Text = (things[i].Parameter == 0) ? string.Format("  h:{0}", things[i].Position.z) : string.Format("  z:{0}", things[i].GetFlagsValue());
									renderer.RenderText(tl2);
								}
							}
						}
					}
					else if (things.Count == 1)
					{
						...
						tl.Rectangle = new RectangleF(things[0].Position.x, things[0].Position.y, 0f, 0f);
						tl.Text = "V";
						renderer.RenderRectangleFilled(new RectangleF(t.Position.x - vsize, t.Position.y - vsize, vsize * 2.0f, vsize * 2.0f), valColor, true);
						renderer.RenderText(tl);
						if (showheightz)
						{
							TextLabel tl2 = vertexnames[4];
							tl2.Rectangle = new RectangleF(things[0].Position.x, things[0].Position.y, 0f, 0f);
							tl2.Text = (things[0].Parameter == 0) ? string.Format("  h:{0}", things[0].Position.z) : string.Format("  z:{0}", things[0].GetFlagsValue());
							renderer.RenderText(tl2);
						}
					}
				}

				// Show new points being assigned
				if (mode == MODE_ASSIGNGROUPS)
				{
					for (int i = 1; i < points.Count; i++)
						renderer.RenderLine(points[i - 1].pos, points[i].pos, LINE_THICKNESS, valColor, true);
					if (points.Count > 0)
					{
						if (things != null && things.Count == 1)
						{
							renderer.RenderLine(points[points.Count - 1].pos, things[0].Position, LINE_THICKNESS, valColor, true);
							renderer.RenderLine(things[0].Position, points[0].pos, LINE_THICKNESS, valColor, true);
							renderer.RenderRectangleFilled(new RectangleF(things[0].Position.x - vsize, things[0].Position.y - vsize, vsize * 4.0f, vsize * 4.0f), valColor, true);
						}
					}
					for (int i = 0; i < points.Count; i++)
					{
						...
						TextLabel tl = vertexnames[i];
						tl.Rectangle = new RectangleF(points[i].pos.x, points[i].pos.y, 0f, 0f);
						tl.Text = (i + 1).ToString();
						renderer.RenderRectangleFilled(new RectangleF(v.x - vsize, v.y - vsize, vsize * 2.0f, vsize * 2.0f), valColor, true);
						renderer.RenderText(tl);
					}
				}
				...
				if (mode >= MODE_RELPOINTS)
				{
					...
					if (points.Count > 0)
					{
						renderer.RenderLine(points[points.Count - 1].pos, curmousepos, LINE_THICKNESS, valColor, true);
						renderer.RenderLine(curmousepos, points[0].pos, LINE_THICKNESS, valColor, true);
					}
					for (int i = 0; i < points.Count; i++)
					{
						if (i >= 3) break;
		...
		{
			base.OnMouseMove(e);
			if (panning) return; // Skip all this jazz while panning

			if (mode < MODE_RELPOINTS) // Select / Assign
			{
				overgroups.Clear();
				int newOverTagID = -1;
		...
		public override void OnMouseDown(MouseEventArgs e)
		{
			...
			if (mode == MODE_SELECT)
			{
				if (e.Button == MouseButtons.Left)
				{
					...
					if (BuilderPlug.Me.VertexSlopeAssistForm.GetGroupsAt(mousemappos, ref overgroups, true, true) > 0)
					{
						if (!General.Interface.CtrlState) overgroupoffset++;
						BuilderPlug.Me.VertexSlopeAssistForm.TagID = overgroups[overgroupoffset % overgroups.Count];
						if (General.Interface.CtrlState) CB_TagGroup_Modify(this, EventArgs.Empty);
					}
				}
				if (e.Button == MouseButtons.Right)
				{
					overgroups.Clear();
					int vertices = BuilderPlug.Me.VertexSlopeAssistForm.GetGroupsAt(mousemappos, ref overgroups, true, true);
					if (vertices > 0)
					{
						BuilderPlug.Me.VertexSlopeAssistForm.TagID = overgroups[overgroupoffset % overgroups.Count];
						BuilderPlug.Me.VertexSlopeAssistForm.OpenContextGroupMenu(Cursor.Position.X, Cursor.Position.Y, true);
					}
					else
					{
						vertices = BuilderPlug.Me.VertexSlopeAssistForm.VerticesInTag();
			...
			else if (mode == MODE_ASSIGNGROUPS)
			{
				...
				{
					overgroups.Clear();
					int num = BuilderPlug.Me.VertexSlopeAssistForm.GetGroupsAt(mousemappos, ref overgroups, true, false);
					if (num > 1)
					{
						BuilderPlug.Me.VertexSlopeAssistForm.OpenContextSelectVertex(Cursor.Position.X, Cursor.Position.Y, overgroups);
					}
					else if (num == 1)
					{
						BuilderPlug.Me.VertexSlopeAssistForm.TagID = overgroups[0];
					}
				}
				if (e.Button == MouseButtons.Right)
				{
